...
class StreamlitDataService:
    ...
    def get_technical_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        macd_fast = self.config.get(
            "technical_analysis.indicators.macd.fast_period", 12
        )
        macd_slow = self.config.get(
            "technical_analysis.indicators.macd.slow_period", 26
        )
        macd_signal = self.config.get(
            "technical_analysis.indicators.macd.signal_period", 9
        )
        bb_period = self.config.get(
            "technical_analysis.indicators.bollinger_bands.period", 20
        )
        bb_std = self.config.get(
            "technical_analysis.indicators.bollinger_bands.std_dev", 2
        )
        ...
        indicators["BB_Upper"] = indicators["BB_Middle"] + (
            data["Close"].rolling(window=bb_period).std() * bb_std
        )
        indicators["BB_Lower"] = indicators["BB_Middle"] - (
            data["Close"].rolling(window=bb_period).std() * bb_std
        )
