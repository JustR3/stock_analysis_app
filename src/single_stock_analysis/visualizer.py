...
class StockVisualizer:
    ...
    def plot_price_and_moving_averages(
        ...
        # Plot closing price
        sns.lineplot(
            data=data, x=data.index, y="Close", label="Price", color="black", alpha=0.7
        )
    ...
    def plot_technical_indicators(
        ...
        plt.subplot(2, 1, 1)
        plt.plot(
            data.index, data["Close"], label="Close Price", color="blue", linewidth=2
        )

        if (
            "BB_Upper" in indicators.columns
            and "BB_Middle" in indicators.columns
        ...
        if "RSI" in indicators.columns:
            ...
            plt.axhline(
                y=50, color="gray", linestyle="-", alpha=0.3, label="Neutral (50)"
            )
    ...
    def plot_volume_chart(
        self, data: pd.DataFrame, ticker: str, save: bool = False
    ) -> Optional[plt.Figure]:
        """
    ...
    def plot_returns_distribution(
        self, data: pd.DataFrame, ticker: str, period: str = "daily", save: bool = True
    ) -> Optional[plt.Figure]:
        """
        ...
        sns.histplot(
            data=data, x=returns_col, kde=True, stat="density", color="blue", alpha=0.6
        )
        ...
        # Customize plot
        plt.title(
            f"{ticker} {period.capitalize()} Returns Distribution", fontsize=16, pad=20
        )
