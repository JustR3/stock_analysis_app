...
def run_feature_analysis(symbol: str = 'AAPL', days: int = 365):
    ...
    try:
        ...
        if corr_results:
            print(f"📊 Found {corr_results['summary']['highly_correlated_pairs']} highly correlated feature pairs")
        ...
        if recommendations.get('suggested_features'):
            print(f"✅ Suggested features ({len(recommendations['suggested_features'])}):")
        ...
        if recommendations.get('correlation_issues'):
            print(f"⚠️  Correlation issues to address ({len(recommendations['correlation_issues'])}):")
            for issue in recommendations['correlation_issues'][:3]:
                print(f"   • {issue['features'][0]} ↔ {issue['features'][1]} (r={issue['correlation']:.3f})")
        ...
        if corr_fig:
            corr_fig.savefig(f'{symbol}_correlation_matrix.png', dpi=300, bbox_inches='tight')
            print(f"✅ Saved correlation matrix to {symbol}_correlation_matrix.png")

        # Feature importance
        importance_fig = eda.plot_feature_importance(top_n=15)
        if importance_fig:
            importance_fig.savefig(f'{symbol}_feature_importance.png', dpi=300, bbox_inches='tight')
        ...
        print(f"📁 Check {symbol}_correlation_matrix.png and {symbol}_feature_importance.png for visualizations")
